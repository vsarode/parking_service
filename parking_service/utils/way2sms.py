...
class sms:
    ...
    def msgSentToday(self):
        ...
        self.t = self.soup.find("div", {
            "class": "hed"}).h2.text  # div element with class "hed" -> h2
    ...
    def send(self, mobile_no, msg):
        ...
        self.payload = {'ssaction': 'ss',
                        'Token': self.jsid,
                        # inorder to visualize how I came to these payload,
                        'mobile': mobile_no,
    ...
    def sendLater(self, mobile_no, msg, date,
                  time):  # Function for future SMS feature.
        ...
        if int(dateparts[0]) < 1 or int(dateparts[0]) > 32 or int(
                dateparts[1]) > 12 or int(dateparts[1]) < 1 or int(
                dateparts[2]) < 2017 or int(timeparts[0]) < 0 or int(
                timeparts[0]) > 23 or int(timeparts[1]) > 59 or int(
                timeparts[1]) < 0:
            return False
